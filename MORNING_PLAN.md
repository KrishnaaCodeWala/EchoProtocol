# Echo — Final Hackathon Morning Push
...
- Banner at top: `"This is a live Echo agent running on Somnia Testnet. No wallet required to view."`
...
- Redis cache: `echo:inference:{address}` with 1h TTL
...
- On each `explainTrade` call: `redis.incr('echo:stats:aiCalls:today')` with a midnight TTL
- On each `callClosePosition` / `callOpenPosition`: `redis.incr('echo:stats:executions:today')`
- On each swap event evaluated: `redis.incr('echo:stats:evaluated:today')`
...
Sections:
1. **What is Echo?** — 3 sentences (agent-first copy-trading on Somnia)
...
Create a new Next.js project at `/Desktop/code/projects/echo-landing/`.

**Approach:** `npx create-next-app@latest echo-landing --tailwind --app` then edit index page.
...
- [ ] GitHub repo public: `manovHacksaw/echo-app`
...
8. Close: "Every step — discover, deploy, execute, protect — runs without user input. That's Echo on Somnia's Agentic L1."
