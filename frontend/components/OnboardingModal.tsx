...
function getSigningInfo(walletClientType?: string) {
  ...
  if (EMBEDDED_CLIENT_TYPES.has(walletClientType)) {
    return {
      short: 'Signs in Echo',
      detail: 'Embedded wallet (created when you signed in with email/Google). Transaction prompts open in a secure Echo/Privy popup — no browser extension required.',
    };
  }
...
export default function OnboardingModal() {
  ...
  useEffect(() => {
    if (ready && authenticated && address) {
      const seen = localStorage.getItem(`echo_onboarding_seen_${address.toLowerCase()}`);
      if (!seen) {
        setIsOpen(true);
      }
    } else {
  ...
  const handleComplete = () => {
    localStorage.setItem(`echo_onboarding_seen_${address.toLowerCase()}`, 'true');
    setIsOpen(false);
    router.push('/traders');
  };
        ...
        <div key={step} className="min-h-[200px] flex flex-col justify-center animate-fade-in-up">
          ...
          {step === 1 && (
            <div className="space-y-4">
              <h3 className="text-xl font-light text-foreground tracking-tight">Welcome to Echo</h3>
              <p className="text-sm text-muted font-light leading-relaxed">
                Echo automates copy trading on the blockchain. Your account identity has been initialized using Privy.
              </p>
          ...
          {step === 2 && (
            <div className="space-y-4">
              ...
              <ul className="space-y-2.5 text-xs text-muted font-light">
                ...
                <li className="flex items-start gap-2">
                  <span className="text-accent mt-0.5">•</span>
                  <span><strong>aUSD (Echo USD)</strong> is a simulated stablecoin. No real USDC is committed or at risk.</span>
                </li>
                <li className="flex items-start gap-2">
                  <span className="text-accent mt-0.5">•</span>
